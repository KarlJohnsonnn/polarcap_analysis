#!/usr/bin/env python3
"""Spectral waterfall: PNG frames and optional MP4 from YAML + meteogram data.

Entry points
------------
- **Preferred:** ``python scripts/analysis/growth/run_spectral_waterfall.py`` (thin wrapper around this file).
- **This script:** run from repo root or from ``scripts/processing_chain/``; default config is
  ``config/cfg_spectral_waterfall.yaml`` (repo root).

Layout and modes
----------------
- Per station: upper liquid / lower ice rate panels; shared diameter axis; optional PSD twin strip.
- Default sampling follows the **frozen-plume ridge** (one row across stations).
- ``--plot-style``: bars | lines | steps.  ``--yscale``: symlog (default) | linear | log.
- ``--mp4-only``: **no** ``load_process_budget_data``; scans existing PNGs under
  ``output/gfx/png/05/<cs_run>/`` and concatenates with ffmpeg only.

Outputs
-------
- Frames: ``output/gfx/png/05/<cs_run>/exp<eid>/<N|M>/spectral_waterfall_<kind>_..._itime<k>.png``
- MP4: ``output/gfx/mp4/spectral_waterfall_<kind>_..._evolution_nframes<n>.mp4`` (``--mp4`` or after render)
- Growth CSV / footer strip: YAML ``plotting.spectral_waterfall_Q`` / ``_N`` (or legacy ``spectral_waterfall``).
  Ridge/growth keys are inherited from the sibling block if the active kind’s block omits them (publication N+Q).
- **Layout / style:** ``panel_height_ratios``, ``main_*_space``, ``footer_*``, ``axis_tick_label_pt``, ``grid_*``,
  ``panel_annotation_*``, ``growth_cursor_color`` — one recipe for PSD, rate panels, and growth footer (see cfg YAML comments).

Examples
--------
From ``scripts/processing_chain/``, mass **Q** with symlog limits and MP4 (one shell line; wrap with ``\\`` for readability)::

    python run_spectral_waterfall.py --kind Q --linthresh-w 1e-10 --linthresh-f 1e-10 \\
        --xlim-w 1e-1 4e3 --xlim-f 1e-1 4e3 --ylim-w=-1e-2,1e-2 --ylim-f=-1e-2,1e-2 \\
        --psd-ylim-w 1e-6 1e0 --psd-ylim-f 1e-6 1e0 --mp4

From repo root, explicit config and thread workers::

    python scripts/processing_chain/run_spectral_waterfall.py \\
        --config config/cfg_spectral_waterfall.yaml \\
        --workers 4 --kind Q --mp4

Number **N** (negative y-limits: use ``=`` or two-token form so argparse does not eat ``-``)::

    python run_spectral_waterfall.py --kind N --linthresh-w 1e-9 --linthresh-f 1e-9 \\
        --ylim-w=-1,1 --ylim-f=-1,1 --xlim-w 0.001 4000 --xlim-f 0.001 4000 --mp4

MP4 only (frames already on disk; fast path, no Zarr load)::

    python run_spectral_waterfall.py --mp4-only --kind Q

See also ``--help`` for full CLI (normalize mode, station/exp/range overrides, ``--no-psd-twin``, etc.).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python scripts/analysis/growth/run_spectral_waterfall.py --config config/cfg_spectral_waterfall.yaml --mp4\n"
    ...
    parser.add_argument("--config", type=Path, default=REPO_ROOT / "config" / "cfg_spectral_waterfall.yaml", help="Path to cfg_spectral_waterfall.yaml")
